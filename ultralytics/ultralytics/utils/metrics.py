...
def soft_nms(bboxes, scores, iou_thresh=0.5, sigma=0.5, score_threshold=0.25):
    """
    Soft-NMS 实现（Gaussian 形式）
    参数：
        bboxes: Tensor[N, 4]，每个候选框的位置（xyxy格式）
        scores: Tensor[N]，每个候选框的得分
        iou_thresh: IoU阈值，用于判断是否进行得分衰减
        sigma: 控制得分衰减速度的高斯参数
        score_threshold: 得分低于该值的框将被剔除
    返回：
        保留的候选框索引列表（Tensor）
    """
    ...
    while order.numel() > 1:
        # 如果只剩下一个框，直接加入keep后退出
        if order.numel() == 1:
            keep.append(order[0])
            break
        else:
            i = order[0]  # 当前得分最高的框索引
        ...
        if newOrder.numel() == 0:
            break  # 没有得分满足要求的框了，结束
        else:
            # 找出新的得分最高框的索引
            maxScoreIndex = torch.argmax(scores[order[newOrder + 1]])

            # 将新得分最高框放到最前，便于下轮优先处理
            if maxScoreIndex != 0:
                newOrder[[0, maxScoreIndex],] = newOrder[[maxScoreIndex, 0],]

            # 更新待处理的索引列表（不含当前已处理的主框）
            order = order[newOrder + 1]

    return torch.LongTensor(keep)  # 返回保留框索引
