...
if __name__ == '__main__':
    # 断点续练
    # model = YOLO("/root/autodl-tmp/ultralytics-main/runs/detect/train9/weights/last.pt")
    # model.train(resume=True)

    # 加载模型
    # model = YOLO("ultralytics/cfg/models/v8/yolov8-Bi-deformable-seg.yaml")  # 从头开始构建新模型
    model = YOLO("runs/detect/v8bide/weights/best.pt")  # 加载预训练模型（推荐用于训练）

    # Use the model
    # results = model.train(data="ultralytics/cfg/datasets/mydatasets-seg.yaml", epochs=130, batch=1, imgsz=640 ,iou=0.4)  # 训练模型
    results = model.val(data="ultralytics/cfg/datasets/mydatasets-seg.yaml")  # 在验证集上评估模型性能
    # results = model("vitiligo_detection01/images/test/img_t_10_0105-jpg_rotated_270_jpg.rf.a803d96d00f15169c9466cd341c9b8dd.jpg")  # 预测图像
...
#
# # Start training from a pretrained 00*.pt model
