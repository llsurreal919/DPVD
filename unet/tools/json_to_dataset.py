...
2、此处生成的标签图是8位彩色图，与视频中看起来的数据集格式不太一样。
   ...
   所以其实和视频中VOC数据集的格式一样。因此这样制作出来的数据集conda install conda-forge::labelme是可以正常使用的。也是正常的。
'''
if __name__ == '__main__':
    jpgs_path   = "datasets_road/JPEGImages"
    pngs_path   = "datasets_road/SegmentationClass"
    # classes     = ["_background_","person", "car", "motorbike", "dustbin","chair","fire_hydrant","tricycle","bicycle","stone"]
    classes     = ["_background_","car","dashedline","midline","pothole","rightlane"]
    
    count = os.listdir("datasets_road/before/")
    for i in range(0, len(count)):
        path = os.path.join("datasets_road/before", count[i])
