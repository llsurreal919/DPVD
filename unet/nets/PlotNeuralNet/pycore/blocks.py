...
# define new block
def to_Attention(name, to_pos, s_filer, n_filer, width, height, depth, opacity_val):
    return [
        to_Conv(name=name + "_conv", s_filer=s_filer, n_filer=n_filer, to=to_pos, width=width, height=height,
                depth=depth),
        to_Pool(name=name + "_pool", to=to_pos, width=width, height=height, depth=depth, opacity=opacity_val),
        to_connection(name + "_pool", name + "_conv")
    ]
